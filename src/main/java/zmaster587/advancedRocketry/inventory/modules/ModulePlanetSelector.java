...
public class ModulePlanetSelector extends ModuleContainerPan implements IButtonInventory {
	...
	private void renderPlanets(DimensionProperties planet, int parentOffsetX, int parentOffsetY, int parentRadius, float distanceMultiplier, float planetSizeMultiplier) {
		...
		int offsetX = parentOffsetX + (int)(Math.cos(planet.getOrbitTheta())*((planet.getOrbitalDist()*distanceMultiplier) + parentRadius)) - displaySize/2;
		int offsetY = parentOffsetY + (int)(Math.sin(planet.getOrbitTheta())*((planet.getOrbitalDist()*distanceMultiplier) + parentRadius)) - displaySize/2;
